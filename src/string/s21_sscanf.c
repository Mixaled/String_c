...
void getNextFormatElem(char *input, char elem[8192], int *checkStartScanf,
  ...
  static char *currentFormatString = s21_NULL;
  ...
  if (!(*startParse) && *currentFormatString != '%') {
    ...
    *startParse = true;
    ...
    if (*currentFormatString != '\0') {
      ...
      currentFormatString = s21_NULL;
    }
  }
  if (returnStr != s21_NULL) {
    s21_strcpy(elem, returnStr);
  } else {  // if return is s21_NULL then we return "error" string
...
char formatParsing(char formatStatic[16384], char currentFormatElem[8192],
  ...
  if (s21_strcmp(currentFormatElem, OUR_ERROR_) != 0) {
    *formatLoaded = true;
    ...
    char nextSym[2];
    s21_sprintf(nextSym, "%c", *currentFormatElem);
...
void chopLeft(char input[16384], int howMany, s21_size_t *n_counter,
  ...
  int j = 0, counter = 0;
  s21_size_t length = s21_strlen(input);
